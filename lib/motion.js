...
class Motion extends Subdevice {
	...
	onmessage(msg) {
		...
		if ('lux' in msg.data) this.status.lux = msg.data.lux;

		if (msg.cmd === 'report') {
			// { cmd: 'report', model: 'motion', sid: '158d0000f085ee', short_id: 37841, data: { status: 'motion' } }
			...
			// { cmd: 'report', model: 'sensor_motion.aq2', sid: '158d0001e546da', short_id: 3639, data: { status: 'motion' } }

			if ('status' in msg.data && msg.data.status === 'motion') {
				this.status.last_motion = Date.now();
				this.status.motion = true;
			}
			else if ('no_motion' in msg.data) {
				this.status.motion = false;
				this.status.no_motion = msg.data.no_motion;
			}
		}
