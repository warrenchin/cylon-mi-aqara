...
class Gateway extends Subdevice {
  ...
  onmessage(msg) {
    ...
    if ('data' in msg) { // iam msg do not have data
      if ('rgb' in msg.data) this.prop.rgb = msg.data.rgb;
      if ('illumination' in msg.data) this.prop.illumination = msg.data.illumination;
    }
    ...
    } // handled by aqara-interface
    else if (msg.cmd === 'get_id_list_ack') {
    } // handled by aqara-interface
    else return false;
