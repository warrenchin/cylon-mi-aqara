...
class Magnet extends Subdevice {
	...
	onmessage(msg) {
		...
		if ('status' in msg.data) {
			let current = msg.data.status;
			this.status.status = current;
			this.status['last_' + current] = Date.now();
		}

		if (msg.cmd === 'report') {
		}
		else if (msg.cmd === 'heartbeat') {
			// { cmd: 'heartbeat', model: 'magnet', sid: '158d0000d619b5', short_id: 62530, data: { voltage: 3055, status: 'close' } }
		}
		else if (msg.cmd === 'read_ack') {
			// { cmd: 'read_ack', model: 'magnet', sid: '158d0000d619b5', short_id: 62530, data: { voltage: 3055, status: 'close' } }
		}
		else return false;
